...
adc_cali_handle_t batt_voltage_adc_cali_handle{};

void adc_calibration_init(adc_unit_t unit, adc_channel_t channel, adc_atten_t atten, adc_bitwidth_t bitwidth,
...
std::unique_ptr<HAEntity> create_battery_entity()
{
        ...
        */
        auto battery_voltage_string = [] {
                auto const voltage = get_battery_voltage();
                auto result = std::to_string(voltage);
                result.resize(8); // Truncate decimals, assuming left of the decimal point is less than 7 digits.
                return result;
        }();
        entity->state = std::move(battery_voltage_string);
